...
// Creates nft with supplied parameters and returns its id
async function createNewNft({
    ...
    //sign and submit NFT
    const createNftTransactionSign = await createNftTx.sign(treasuryPk);
    const createNftSubmit = await createNftTransactionSign.execute(client);
...
// Mint token(s) and return receipt(s)
...
// Token transaction to an account
...
async function main() {
    // Get accounts ids and private keys
    ...
    //Create NFT, mint 5 and log them
    ...
    console.log(`\n- NFT transfer from Treasury to Account3: ${tokenTransferReceipt.status} \n`);
